...
2. **Azure Files tiers** (Transaction optimized / Hot / Cool + Premium). citeturn0search11turn0search8
...
For **standard** (non-premium) blob data, Azure offers multiple access tiers. Key characteristics: citeturn0search8turn0search13
...
For **standard SMB Azure file shares**, Azure offers **three access tiers**: citeturn0search11
...
Note: **Auto-tiering from Cool to Hot on access is limited to once every 30 days** to reduce early-deletion penalties and unexpected cost. citeturn0search13turn0search7
...
- **Soft-deleted** data and previous versions are **billed like active data** while retained (for both blobs and files). citeturn1search2turn1search0turn0search0
...
You can implement this using **blob lifecycle management** rules (see dedicated note). citeturn0search14turn0search31
