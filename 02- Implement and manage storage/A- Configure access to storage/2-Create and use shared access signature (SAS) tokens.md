...
- Anyone can read, write, delete, or list all data in supported services (Blob, Files, Queues, Tables). citeturn0search3

To reduce this risk, Azure provides **Shared Access Signatures (SAS)**:

- SAS = a **signed URL** or token that grants **limited access** to a resource for a **limited time**, with specific **permissions**. citeturn0search1turn0search16turn0search24  
...
Common use cases: citeturn0search1turn0search21
...
There are **three main SAS types** you need to know: citeturn0search1turn0search11turn0search16
...
- **Signed with a user delegation key**, which is obtained using **Microsoft Entra (Azure AD)** credentials. citeturn0search6turn0search11turn0search16  
- Applies to **Blob service only** (containers/blobs). citeturn0search11  
- Considered the **most secure** SAS type because:
  - You do **not** use account keys.  
  - Permissions are tied to the identity’s RBAC roles. citeturn0search6turn0search1turn0search21

Creation steps (conceptual): citeturn0search6turn0search16
...
- Grants access to **specific resources** of a single service (Blob, File, Queue, or Table). citeturn0search1turn0search16  
...
- Grants access at the **account level** across one or more services (Blob, File, Queue, Table). citeturn0search1turn0search16  
...
Common fields (names may evolve with API versions): citeturn0search16turn0search24
...
- SAS always includes **expiry**, **permissions**, and **signature**. citeturn0search1turn0search24  
...
2. Click **Generate SAS** or **Generate SAS URL**. citeturn0search16  
3. Choose:
...
1. Go to **Storage account → Security + networking → Shared access signature**. citeturn0search1turn0search16  
2. Specify:
...
Microsoft and security guidance highlight several best practices: citeturn0search1turn0search21turn0search24
...
3. **Limit the validity period.**
   ...
   - You can also enforce **SAS expiration policies** at the account level. citeturn0search24
...
6. **Use stored access policies for revocation and governance.**
   - Instead of handing out a SAS with its own embedded expiry, tie SAS to a **stored access policy** so you can revoke or change permissions centrally. citeturn0search2turn0search7
...
A **stored access policy** is created on a container, file share, queue, or table and defines: citeturn0search2turn0search7
...
- Use **user delegation SAS** for the Blob container: app authenticates using Entra ID, requests a SAS to allow `w` (write) and `c` (create) for 15 minutes. citeturn0search6turn0search11turn0search16  
...
- Create a **stored access policy** on the container with `r` permission and 3-day expiry. citeturn0search2turn0search7  
...
- Use **Account SAS** with services = Blob + Queue, resource types = service/container/object, read/list/process permissions. citeturn0search1turn0search16  
...
- Delete or modify the **stored access policy** that those SAS tokens reference → all tokens referencing that policy become invalid. citeturn0search2turn0search7  
- If the SAS was created **without** a stored access policy, you must **regenerate the account key** (breaking all SAS and any app using that key). citeturn0search3

---

## I. Exam takeaways

- **SAS = limited, time-bound access:** always think “least privilege, limited time, specific resource.” citeturn0search1turn0search21turn0search24  
- **User delegation SAS** is preferred for Blob + Entra-based scenarios. citeturn0search6turn0search11turn0search16  
- **Service SAS** targets specific resources in one service; **Account SAS** can span multiple services. citeturn0search1turn0search16  
- Use **stored access policies** when you need revocation and governance of long-lived SAS. citeturn0search2turn0search7  
- Use **HTTPS only**, short lifetimes, and least privilege permissions; restrict by IP range when possible. citeturn0search21turn0search24
