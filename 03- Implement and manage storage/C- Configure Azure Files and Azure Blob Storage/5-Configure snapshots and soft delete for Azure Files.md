...
2. **Soft delete for file shares** – protects shares from being permanently deleted. citeturn0search16turn0search15turn0search17
...
Key properties: citeturn0search16
...
Snapshots are **never overwritten**, so you must delete them manually or with automation when no longer needed. citeturn0search16
...
**Soft delete for Azure Files** protects file shares from being **permanently deleted**. When soft delete is enabled: citeturn0search15turn0search20turn0search17
...
- It does **not** protect individual files from being deleted or modified – for that you use **snapshots or backup**. citeturn0search17
...
4. Choose retention (1–365 days). Many security guides recommend at least **7 days**. citeturn0search1turn0search9turn0search20
5. Save changes.

After enabling, all file shares in that storage account are protected by soft delete.

> Note: When you configure **Azure Backup** for file shares, the backup service may automatically enable soft delete with a default retention (e.g., 14 days) to protect backup data. citeturn0search5turn0search5
...
Azure restores the share and all its data (including snapshots) as it was at the time of deletion. citeturn0search15turn0search1

### 4. Billing Behavior & Purging

From Microsoft docs and community Q&A: citeturn0search0turn0search29
...
| **Azure Backup** | Files, shares, multiple restore points | Ransomware, long‑term retention, DR | Policy-based (days/years) | Full backup & compliance | citeturn0search5turn0search11
...
High-level behavior (relevant to exam): citeturn0search5
