...
- Can be **swapped** with the production slot with minimal downtime. citeturn3search1

Slots are available for apps in **Standard, Premium, and Isolated** tiers. Free/Shared/Basic do **not** support slots. citeturn3search1turn1search1
...
2. In the left menu, go to **Deployment → Deployment slots**.
   - If you’re not on Standard or higher, you’ll see an **Upgrade** prompt. citeturn3search1
3. Click **Add**.
...
- Initially, it has **no content** – you deploy code to this slot just like to any other app. citeturn3search1
...
**Swapping** exchanges app content and some configuration between two slots (usually staging ↔ production) with minimal downtime. citeturn3search1
...
**Swap with preview** lets you preview how configuration will behave before completing the full swap. citeturn3search1
...
(Check latest CLI docs for the exact `--action` values.) citeturn3search1

---

## F. What configuration is swapped vs not swapped

When you swap slots, **some settings move with content**, others stay with the slot. citeturn3search1turn3search6
...
- Deployment slot-specific settings (Always On, some diagnostic settings, IP restrictions, etc.) citeturn3search1
...
→ Mark the database connection string as a **slot setting** so it isn’t swapped. citeturn3search6turn3search2

---

## G. Auto swap

**Auto swap** automatically swaps from a source slot (e.g., staging) into a target slot (usually production) after each deployment, once the app is warmed up. citeturn3search1turn3search3
...
App Service lets you **route a percentage of traffic** to a non‑production slot for A/B testing or canary releases. citeturn3search1
...
3. **Plan tier and limits**
   ...
   - Each tier has a maximum number of slots (e.g., Standard supports 5). citeturn3search1turn1search1
...
5. **Use staging slot for restoring backups**
   - Restore backups to a slot first, validate, then swap to production. citeturn1search3turn1search18
