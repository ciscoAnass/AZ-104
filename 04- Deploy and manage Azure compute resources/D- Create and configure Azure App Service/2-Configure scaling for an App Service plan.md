...
Azure App Service supports two main types of scaling: citeturn1search2turn1search24
...
- When you scale the plan out to 5 instances, *all apps* in that plan now run on those 5 instances. citeturn1search1turn1search9

In some premium tiers, you can also enable **per‑app scaling** to limit how many instances a single app uses. citeturn1search20
...
- You can typically **scale back down** later, but ensure the target tier supports your current number of slots or features. citeturn1search2turn3search1
...
Newer portal UX exposes autoscale under **Scale‑out (App Service plan)** / **Automatic**. citeturn1search6turn1search24
...
Azure uses App Service plan’s autoscale engine (under the hood this uses the Azure Monitor autoscale feature). citeturn1search24

### Key exam parameters / options

- **Minimum instances** – ensures some baseline capacity (e.g., 2 instances).
- **Maximum scale limit** – cap the cost (e.g., no more than 10 instances). citeturn1search6
...
By default, when the plan scales, **all apps scale together**. With **per‑app scaling**, an app can be limited to fewer instances than the plan. citeturn1search20
...
Per‑app scaling is available only in **Standard, Premium, PremiumV2, PremiumV3, and Isolated** tiers (see docs for precise current support). citeturn1search20
...
4. **Use pre‑warmed instances for PremiumV3/PremiumV4**
   - Always‑ready / pre‑warmed instances help reduce cold starts for HTTP traffic. citeturn1search6

5. **Remember plan‑level impact**
   - Scaling out might help one app but also increases capacity (and cost) for **all** apps in the plan.

6. **Monitor costs**
   - Use Azure Cost Management + **budgets** and alerts to avoid unexpected bills. citeturn1search27turn1search10
